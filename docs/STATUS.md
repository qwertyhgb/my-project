...
1. **G0-R**：先 `--dry-run`，再运行自动 QC（**draft-0.3**；旧失败运行目录不得覆盖），按 `qc_report.md` 冻结 `frozen_decision`
