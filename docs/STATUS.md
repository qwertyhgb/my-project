...
| 项 | 值 |
|---|---|
| 最后更新 | 2026-09-20（**版本控制已启用：Git 基线 commit `23ca6cb`，B1 关闭**；PZ/TZ 链路 6 轮加固 + seg 哨兵兼容 + `--overfit` 运行时显存遥测；研究者已执行：3 例 prior dry-run + 全量 1500 例 prior 物化（B10 关闭）；**G0-R Automated：draft-0.2 真实运行失败（见 B2）→ draft-0.3 → draft-0.4 修复（合成测试 52 项）→ draft-0.4 真实 16 例运行已由研究者执行完成**（2026-09-18 08:58–09:16，`outputs/diagnostics/g0_r_automated/20260918_085811/`，用时 1102.8 s：类型错误 **0/32** 已消除，但两个 pair 主指标检出率 0.667 < 0.9 → 校准不通过，候选 `INSUFFICIENT_EVIDENCE`，**`frozen_decision` 未填写**）；**2026-09-19→09-20：研究者启动并完成正式 N0 基线训练**（`nnUNetTrainerPICAI_FLCE_NoFFT`；1000 epoch / 15.4 h / `Training done` / `checkpoint_final.pth`；`Mean Validation Dice = 0.1682`；身份 = feasibility run，详见 `docs/experiment_log.md`）；**2026-09-20：M0–M4 预算 200 → 1000 epoch、全体积验证 5 → 50（预注册，见 `docs/protocol_changelog.md`；B7 的 epoch 部分已对齐，骨干差异仍待 D7）**；日常入口 `docs/START_HERE.md`；全部启动前置统一降级为「不影响启动、只影响结果标签」） |
...
| **G0-R**（序列错位协议冻结） | **PENDING / DRAFT**（门状态**未变**） | 抽样已执行：`outputs/diagnostics/g0_r/20260915_093819/`（`decision=null`、`status=PENDING`）；**draft-0.2 真实运行已执行但失败**：`outputs/diagnostics/g0_r_automated/20260918_074219/`（16 例 × 2 对，1072.7 s，32/32 `REGISTRATION_DIAGNOSTIC_FAILED`、`calibration_passed=false`、候选 `INSUFFICIENT_EVIDENCE`；产物原样保留）；**draft-0.4 修复已实现 + 合成测试通过**（`tests/unit/test_g0_r_automated_qc.py` 52 passed；修复两项 fail-open：指标 `sigma*.usable` 守卫、校准 unit 条件网格完整性；协议 `protocol.version=draft-0.4`、`rule_version=0.4`、`output_schema_version=g0-r-automated/0.4`、`protocol_hash=13e7be50…`；draft-0.2/0.3 配置均按字节归档；`--dry-run` 已通过且不创建输出目录）；**draft-0.4 真实 16 例运行已由研究者执行完成**：`outputs/diagnostics/g0_r_automated/20260918_085811/`（16 例 × 2 对 = 32 case×pair、跳过 0、用时 1102.8 s、`config_sha256=571cc6a3…` 与当前配置一致、协议哈希 `13e7be50…`；`REGISTRATION_DIAGNOSTIC_FAILED` **0/32**、`INVALID_INPUT` 与 FOV 异常均 0；`calibration_passed=false`（两 pair 主指标 `detection_rate@2.0mm = 0.667 < 0.9`）；候选 `INSUFFICIENT_EVIDENCE`；产物原样保留；见 `docs/experiment_log.md`） | **`frozen_decision` 未填写**（自动候选 `INSUFFICIENT_EVIDENCE`）。**已排除的 D2 选项**：① 人工 landmark 复核 = **不可行**（研究者声明 2026-09-19）；② 更换主指标 = **实测关闭**（三个可用指标的 `detection_rate@2.0mm`：主指标 0.667 / 0.667、`edge_f1_at_2.0mm` 0.667 / 0.667、`chamfer_mm` 0.778 / 0.444，**无一达 0.9**）。**未决前置（2026-09-19 只读核查新发现）**：校准 unit 规则为「从 manifest 顺序取前 `max_cases=3` 例」，而 manifest 第一层即 `geometry_suspect`（`selection_order` 0–4）→ **校准 3 例（10057 / 10161 / 10489）全部是 P0A `severity=2` 几何异常病例**，故「主指标对 2 mm 不敏感」被几何异常混杂，暂不能视为指标的固有属性；是否按预注册协议修正校准 unit 选取（draft-0.5）属 **D2**，由研究者决定（`docs/runbooks/g0_r_alignment_qc.md`） |
...
| **G1**（官方基线可复现） | **N0 训练已完成（identity = feasibility run；三项前置未冻结）** | `nnUNetTrainerPICAI_FLCE_NoFFT__nnUNetPlans__3d_fullres/fold_0/`：2026-09-19 11:15 → 09-20 02:37（**≈15.4 h**，1000 epoch，≈55 s/epoch），日志出现 `Training done`，产出 `checkpoint_final.pth`（341 MB）并完成 223 例 validation（02:48）；**`Mean Validation Dice = 0.1682`**（IoU 0.1235）；`Pseudo dice` 首个非零在 **epoch 43**，最后 200 epoch 全部非零（最高 0.767）；**质量偏弱**：181/223 例预测为空、63 个 GT 阳性例中 **32 例完全漏掉**（逐例 FN 1546 > FP 512 → 欠分割）；既有两个旧 run 与日志原样保留 | ① 研究者执行评测与诊断（`scripts/evaluate/evaluate_n0_validation.py`、`scripts/train/diagnose_n0_zero_dice.py`）；② 若要写成"官方基线复现"，仍须完成 G0-R / G0-E 冻结（本 run 为 feasibility 身份）；③ 是否延长训练/改预算属 §20.2 D7 |
...
| `scripts/evaluate/` 评测逻辑 | **基础评测已实现**（`evaluate_n0_validation.py`：nnU-Net validation vs GT，输出 Dice / 检出 / 患者级混淆 / 基于分数的代理 AP·AUC；8 项合成测试通过）；**官方口径（lesion-level AP / PI-CAUC）仍未实现**（`picai_eval` 未安装，且须按 G0-SAP 先冻结） | `scripts/evaluate/evaluate_n0_validation.py`、`tests/unit/test_evaluate_n0_validation.py`；口径定义见 `docs/protocols/G0_SAP.md` §9 |
...
| **G0-R Automated（自动对齐 QC）** | **draft-0.4 已实现 + 合成测试通过（52 项）；真实运行 2 次：draft-0.2（失败：32/32 类型错误 + 校准不通过）与 draft-0.4（2026-09-18 08:58–09:16，`20260918_085811`：类型错误 **0/32**、两 pair 主指标检出率 0.667 < 0.9 → 校准不通过、候选 `INSUFFICIENT_EVIDENCE`）；产物均原样保留；`frozen_decision` 未填写** | `src/.../protocols/g0_r_automated_qc.py`、`scripts/audit/run_picai_alignment_qc_automated.py`；配置 `configs/protocols/g0_r_alignment_qc_automated.yaml`（draft-0.4；draft-0.2/0.3 归档于 `configs/protocols/archive/`）；协议 `docs/protocols/G0_R_ALIGNMENT_QC_AUTOMATED.md`；运行事实见 `docs/experiment_log.md` |
...
| B2 | G0-R 未冻结：draft-0.2 真实运行失败（工具缺陷：SimpleITK 类型强转）→ draft-0.3 → **draft-0.4** 修复后，**真实 16 例运行已由研究者执行完成（2026-09-18，`outputs/diagnostics/g0_r_automated/20260918_085811/`）**：类型错误 **0/32**（缺陷已消除）、`INVALID_INPUT` 与 FOV 异常均 0；但两个 pair 主指标 `sigma1.5.edge_f1_at_1.0mm` 的 `detection_rate@2.0mm = 0.667 < 0.9` → **校准不通过**，候选 `INSUFFICIENT_EVIDENCE`，**`frozen_decision` 未填写**（draft-0.3 从未在真实数据上运行） | **仍无法确认「仅重采样」是否足够**；自动路径当前结论为「证据不足」，需 **D2** 决策（人工 landmark 复核 / 更换主指标 / 扩大抽样）；P2B / 正式 N0 / M0 在冻结前不得启动 | 研究者按 `docs/runbooks/g0_r_alignment_qc.md` 与 `docs/research_plan.md` §14.1 / §20.2 D2 决定冻结内容 |
...
| B7 | **部分处置（2026-09-20）**：M0–M4 的 epoch 预算已由 200 → **1000**（与 N0 对齐）、验证调度 5 → 50；**骨干差异仍在**（N0 = PlainConv，M0–M4 = Residual-Encoder） | `N0 − M0` 的 **epoch 预算混杂已消除**；**骨干混杂仍在** → 差异仍不能作单一因果解释 | 待冻结决策表 D7（剩余选项：增加 matched-protocol run）；登记见 `docs/protocol_changelog.md` 2026-09-20 |
