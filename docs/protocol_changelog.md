...
| （**前置规定作废声明**，非条款修订） | 2026-09-19 | 研究者声明：① 所有操作暂停；② `research_plan` §14/§14.1 与 runbook 中「冻结前不得启动 P2B / 正式 M0 / N0」等**顺序与前置约束作废**（不再作为启动前置）；③ 门定义、预注册数值、数据与产物均未改动 | 是（仍无任何正式模型结果） | 无科学结果可影响；但**未冻结状态下产出的结果证据等级仍为 exploratory / feasibility**，不得表述为「官方基线复现 / 确认性比较 / H3 独立确认」（见 `docs/PAUSE_AND_VOID_20260919.md` §3） |
...
## 前置规定作废声明（2026-09-19；**非条款修订，研究者决定**）

- **性质**：这是研究者对本项目**执行约束**的处置声明，**不是**对研究设计、门定义或预注册数值的修订；
- **内容**：
  1. **所有操作暂停**（代理不再启动任何新操作），直到研究者解除；
  2. `docs/research_plan.md` §14 / §14.1（含 G0-R 行「冻结前不得启动 P2B、正式 M0 或 N0」）与
     `docs/runbooks/n0_training.md` §1、`docs/runbooks/p2b_m0_validation.md`、`docs/STATUS.md` §4.1 中的
     **顺序 / 前置约束作废**，不再构成任何 run 的启动前置；
  3. `docs/STATUS.md` §3 的 B2 / B3 / B4 / B6 / B7 / B8 / B9 不再阻塞启动；
- **未改变**：门定义与通过条件（`research_plan` §14）、预注册数值（位移/方向/阈值/检出率/显式冻结参数）、
  数据与全部既有产物、`docs/research_plan.md` 文本本身（该文件按研究者长期指令未修改）；
- **是否在查看正式模型结果之前**：是（仍无任何正式模型结果；M0–M4 未训练、未评测）；
- **对既有结果的影响**：无科学结果可影响；**但**未冻结状态下产出的结果只能作为 exploratory / feasibility，
  不得表述为「官方基线复现」「确认性比较」或「H3 独立确认」；升级为确认性证据须在**看结果前**重新冻结并
  在此登记（事后升级必须显式标注并相应降低结论强度）；
- **记录载体**：`docs/PAUSE_AND_VOID_20260919.md`（含被作废条目、原样保留清单、撤销方法）。
