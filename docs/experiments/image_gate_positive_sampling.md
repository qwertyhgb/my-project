...
- patch `16 × 320 × 320`，target spacing `[3.0, 0.5, 0.5]` mm（1 体素 = 0.75 mm³），
...
- **逐 epoch 波动很大**（如 epoch 33 已到 0.6277、epoch 500 低至 0.2650），单点值不可作为
  进展判据；判据应看分箱均值或 EMA。
- **loss 与指标解耦**：train/val loss 全程在 0.001–0.004 的小区间内，与 pseudo dice 无单调关系，
  按 `val_loss` 选模型会选到背景解；模型选择只看 EMA patch pseudo dice。
...
|---|---:|---:|---:|---:|---:|---:|
| 0–1000（<0.75 mL） | 16 | 7 | 0.272 | 0.202 | 1.345 | 0.232 |
| 1000–5000（0.75–3.75 mL） | 27 | 16 | 0.226 | 0.779 | 0.290 | 0.350 |
| 5000–20000（3.75–15 mL） | 15 | 13 | 0.249 | 0.834 | 0.299 | 0.384 |
| >20000（>15 mL） | 5 | 4 | 0.495 | 0.879 | 0.564 | 0.634 |

> 最小档（<0.75 mL）的 `pred/ref = 1.345`、精确仅 0.202：该档的"检出"里混有明显**过预测**，
...
事实陈述（`Research_Plan.md` §5.2 / §11.5 的解释边界）：

1. 门控**已明显偏离恒等映射**（Frobenius 0 → final 0.4940），不是训练后仍 `scales ≡ 1`。
2. 偏置呈现 **T2W > ADC > HBV** 的次序（final +0.177 / +0.066 / −0.243；best 同一次序），
   即 softmax 先验上倾向提高 T2W 缩放、压低 HBV 缩放；best 存档下该倾向更强。
3. HBV 对应输出通道的行范数在两个存档中都最大（final 0.3897、best 0.4550），说明门控主要
   在对 HBV 通道做逐体素的空间调节。
4. **解释边界（`Research_Plan.md` §5.2 / §11.5）**：以上是模型内部学习到的"相对序列
   偏好/尺度"，**不是**经过校准的可靠度、真实图像质量、因果贡献，也不等同于放射科医师判读
   权重；具体偏好只能在逐位置的 softmax 权重图上分析（需一次前向，见待办）。权重高低不能
   单独解释最终分割的任何变化。
...
5. **gate 确实学到了非恒等的序列缩放**（见第 6 节），偏好次序 T2W > ADC > HBV、空间调节
   集中在 HBV 通道；但这只是内部相对尺度，没有可靠度 / 因果含义。
6. **`checkpoint_best` 从未做全 volume 验证**：best（epoch 698）与 final（epoch 999）哪个在
   完整 223 例上更好，本记录无法判断；第 5 节全部数字只来自 final。
7. **训练在 epoch 上限处并未进入稳定平台**：pseudo dice 全程大幅震荡（单 epoch 0.16–0.76），
   1000 epoch 是否足够、更长训练是否改变结果，本实验不能说明。
